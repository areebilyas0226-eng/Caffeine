...
// ─── MENU ITEM ROW (extracted so useState is called at component top-level) ───

function MenuItemRow({ name, price, avail, cat }) {
  const [available, setAvail] = useState(avail);
  return (
    <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "12px", padding: "14px 16px", display: "flex", justifyContent: "space-between", alignItems: "center", opacity: available ? 1 : 0.5 }}>
      <div>
    ...
    return (
      <div style={{ padding: "24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "8px" }}>
          {MENU_ITEMS.map(([name, price, avail, cat], i) => (
            <MenuItemRow key={i} name={name} price={price} avail={avail} cat={cat} />
          ))}
        </div>
      </div>
    );
